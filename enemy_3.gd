...
@onready var movement_speed: float = 50.0
...
func tween_hit(x):
	...
	tween.tween_property(sprite_2d, "position", Vector2(-x,0), 0.08).from_current()
	tween.tween_property(sprite_2d, "position", Vector2(x,0), 0.08).from_current()
	tween.tween_property(sprite_2d, "position", Vector2(-x,0), 0.08).from_current()
	tween.tween_property(sprite_2d, "position", Vector2(), 0.1225).from_current()
...
func _physics_process(delta):
	...
	if state == SEEK:
		...
		if color == BLUE:
			...
			update_target(set_target("player2"), 50.0)
	if state == IDLE:
